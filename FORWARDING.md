...
- Subsystem support: `client-core` drives `ssh-core::session::run_subsystem`, so `rb --subsystem sftp host` opens the requested subsystem instead of starting a shell and pipes its stdio back to the user.
