...
pub fn encrypt_string(value: SecretBoxedString) -> ServerResult<String> {
...
pub fn decrypt_string_if_encrypted(value: &str) -> ServerResult<(SecretBoxedString, bool)> {
    if let Some(rest) = value.strip_prefix(ENC_PREFIX_V2) {
        ...
        return String::from_utf8(pt.expose_secret().clone())
            .map(|s| (SecretBoxedString::new(Box::new(s)), is_legacy))
        ...
        return String::from_utf8(pt.expose_secret().clone())
            .map(|s| (SecretBoxedString::new(Box::new(s)), true))
            .map_err(|_| ServerError::Crypto("decrypted value is not valid UTF-8".to_string()));
    }

    Ok((SecretBoxedString::new(Box::new(value.to_string())), false))
}
...
pub fn encrypt_string_with(value: SecretBoxedString, master: &[u8]) -> ServerResult<String> {
...
pub fn decrypt_string_with(value: &str, master: &[u8]) -> ServerResult<SecretBoxedString> {
    ...
    String::from_utf8(pt.expose_secret().clone())
        .map(|s| SecretBoxedString::new(Box::new(s)))
        .map_err(|_| ServerError::Crypto("decrypted value is not valid UTF-8".to_string()))
}
