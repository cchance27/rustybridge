...
pub async fn list_groups() -> Result<Vec<GroupInfo<'static>>, ServerFnError> {
    ensure_group_claim(&auth, ClaimLevel::View)?;
    server_core::list_groups_overview()
        .await
...
pub async fn create_group(name: String) -> Result<(), ServerFnError> {
    ...
    server_core::add_group(&audit.0, &name)
        .await
        .map_err(|e| ServerFnError::new(e.to_string()))
}
...
pub async fn delete_group(id: i64) -> Result<(), ServerFnError> {
    ...
    server_core::remove_group_by_id(&audit.0, id)
        .await
        .map_err(|e| ServerFnError::new(e.to_string()))
}
...
pub async fn list_group_members(id: i64) -> Result<Vec<String>, ServerFnError> {
    ensure_group_claim(&auth, ClaimLevel::View)?;
    server_core::list_group_members_by_id(id)
        .await
        .map_err(|e| ServerFnError::new(e.to_string()))
}
...
pub async fn get_group_claims(id: i64) -> Result<Vec<ClaimType<'static>>, ServerFnError> {
    ensure_group_claim(&auth, ClaimLevel::View)?;
    server_core::get_group_claims_by_id(id)
        .await
        .map_err(|e| ServerFnError::new(e.to_string()))
}
...
pub async fn add_group_claim(id: i64, claim: ClaimType<'static>) -> Result<(), ServerFnError> {
    ensure_group_claim(&auth, ClaimLevel::Edit)?;
...
pub async fn remove_group_claim(id: i64, claim: ClaimType<'static>) -> Result<(), ServerFnError> {
    ensure_group_claim(&auth, ClaimLevel::Delete)?;
